...
pub enum AnyModel {
    ...
    JetInStar(JetInStar),
    WindShock(WindShock),
    SphericalExplosion(SphericalExplosion),
}
...
impl InitialModel for AnyModel {
    ...
    fn validate(&self) -> anyhow::Result<()> {
        match self {
            ...
            AnyModel::SphericalExplosion(m) => m.validate(),
        }
    }
    ...
    fn primitive_at(&self, coordinate: (f64, f64), time: f64) -> AnyPrimitive {
        match self {
            ...
            AnyModel::SphericalExplosion(m) => m.primitive_at(coordinate, time),
        }
    }
    ...
    fn scalar_at(&self, coordinate: (f64, f64), time: f64) -> f64 {
        match self {
            ...
            AnyModel::SphericalExplosion(m) => m.scalar_at(coordinate, time),
        }
    }
}
